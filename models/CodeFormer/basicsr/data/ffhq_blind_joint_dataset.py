...
class FFHQBlindJointDataset(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        # A benchmark holdout must never leak into training.  The manifest
        # contains POSIX paths relative to dataroot_gt and is generated by
        # tools/prepare_benchmark.py.
        exclude_manifest = opt.get('exclude_manifest')
        if exclude_manifest:
            if not osp.isfile(exclude_manifest):
                raise FileNotFoundError(f'Benchmark exclusion manifest not found: {exclude_manifest}')
            with open(exclude_manifest, 'r', encoding='utf-8') as fin:
                excluded = {line.strip().replace('\\', '/') for line in fin if line.strip()}
            before_count = len(self.paths)
            self.paths = [path for path in self.paths
                          if osp.relpath(path, self.gt_folder).replace('\\', '/') not in excluded]
            logger.info(f'Excluded {before_count - len(self.paths)} benchmark holdout images from training.')
